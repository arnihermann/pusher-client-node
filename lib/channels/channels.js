...
_.extend(Channels.prototype, {
     ...
     */
    add: function(name, channelData, pusher){
        if(!this.channels.hasOwnProperty(name)){
            this.channels[name] = this.createChannel(name, channelData, pusher);
        }
     ...
     * @scope private
     */
    createChannel: function(name, channelData, pusher){
        if(name.indexOf('private-') === 0){
            return new PrivateChannel(name, channelData, pusher);
        }else if(name.indexOf('presence-') === 0){
            return new PresenceChannel(name, channelData, pusher);
        }else{
            return new PublicChannel(name, channelData, pusher);
        }
    }
